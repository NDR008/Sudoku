...
def hidden_pair(sudoku):
    ...
    for (y_pos, x_pos) in zeros:
        ...
        if len(option) == 2:
            #first_position = (y_pos, x_pos)
            #yeah we have the first pair
            for col1 in range(9):
                if col1 == x_pos:
                    continue
                elif valid_options[(y_pos, col2)] == option:
                    #yeah we have the twin
                    #second_position = (y_pos, col2)
                    for col2 in range(9):
                        if col2 != x_pos and col2 != col1:
                            naked_helper(valid_options[(y_pos, col)], option)
                        for (y_pos, x_pos) in zeros:
                            option = valid_options[(y_pos, x_pos)].copy()
                            for col in range(9):
                                if col == x_pos:

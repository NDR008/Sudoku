...
def quick_check_invalid(sudoku):
    zeros = sudoku_zeros(sudoku)
    options = possibilities(sudoku)
    for cell in zeros:
        if cell in options and len(options[cell])<1:
            return False
    return True
...
def sudoku_solver(sudoku):
    ...
    if np.sum(sudoku) < 0 or sudoku_sovled(sudoku):
        return sudoku
    valid_options = partial_naked_tripples(sudoku)
    #valid_options = sort_by_values_len(partial_naked_tripples(sudoku))
    #valid_options = sort_by_values_len(possibilities_pairs(sudoku))
    #valid_options = sort_by_values_len(possibilities(sudoku))
    #valid_options = (possibilities(sudoku))
    zeros = sudoku_zeros(sudoku)

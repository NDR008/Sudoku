...
def get_options_nkd_trpl(sudoku):
    options = get_options(sudoku)
    print(options)
    rows = get_option_row(options)
    for each_row in rows:
        track1 = []
        for key in each_row:
            if 3 >= len(each_row[key]) >= 2:
                track1.append([key,each_row[key]])
        if len(track1) >= 3:
            print(track1)
            for pos1, (ind1,sets) in enumerate(track1):
                for pos2, (ind2,sets2) in enumerate(track1):
                    for pos3, (ind3,sets3) in enumerate(track1):
                        if pos1 != pos2 and pos2 != pos3:
                            result = ((sets | sets2) | sets3)
                            if len(result) == 3:
                                print(sets, sets2, sets3)
                                print(result)
                                for col in range(9):
                                    print
                                    index = (ind1[0], col)
                                    if index in options and index != ind1 and index !=  ind2 and index != ind3:
                                        opts_set = set(options[index]) - result
                                        options[index] = list(opts_set)
    print(options)                                        
            #for key in track1:
                
    return options

def get_option_row(options):
    rows_opts = []
    for y in range(9):
        row_opt = {}
        for x in range(9):
            if (y,x) in options:
                row_opt[(y,x)]=set(options[(y,x)])
        rows_opts.append(row_opt)
    return rows_opts
